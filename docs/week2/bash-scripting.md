...
Правда тут есть один нюанс. Назначенные таким образом алиасы пропадут после перезапуска сессии. Чтобы этого не произошло, нужно их сохранить в файл ~/.bashrc и применить изменения:
...
Создай файл hello.sh со следующим содержимым:
...
Давай теперь запустим этот скрипт. Нужно дать ему права на выполнение: `chmod +x hello.sh` и выполнить: `./hello.sh`

А теперь шок-контент! Сейчас ты узнаешь, как создавать свои собственные команды, такие как `cat`, `pwd` или `ls`. 

Все эти команды хранятся в директориях, перечисленных в переменной окружения `$PATH`, можешь заглянуть внутрь и убедиться: `echo $PATH`. Создай собственную директорию `bin` в своей домашней директории: `mkdir ~/bin`, затем переподключись к серверу: `exit` и зайди снова. Посмотри ещё раз в `$PATH`, теперь там появилось `/home/твоё-имя-пользователя/bin`. Что это значит? А то, что теперь ты можешь поместить свой скрипт в эту директорию и запускать его откуда угодно как команду! 

Перенеси туда `hello.sh` и попробуй вызвать его как команду `hello`.

Давай теперь усовершенствуем скрипт и научим его приветствовать по имени, которое будет передаваться ему на вход.

## Переменные

Убедись, что ты на сервере и открой в Nano файл `/home/твоё-имя-пользователя/bin/hello.sh`:
...
Запусти скрипт с параметрами, например: `~/bin/hello.sh one two`
